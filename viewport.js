...
export default class Viewport {
  ...
  constructor(ctx, zoom) {
    this.ctx = ctx;
    this.scale = 1;
    this.shift = Vec.from([0, 0]);
    this.manualScale = 1;
    this.manualShift = Vec.from([0, 0]);
    this.autoZoom = zoom || 0.9;
    this.minMax = [Vec.from([0, 0]), Vec.from([1, 1])];
  }
